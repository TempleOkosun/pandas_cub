...
class DataFrame:
    ...
    def _check_input_types(self, data):
        ...
        for key, value in data.items():
            ...
            if value.ndim != 1:
                raise ValueError("value of 'data' must be a one-dimensional array")

    def _check_array_lengths(self, data):
        for i, value in enumerate(data.values()):
            if i == 0:
                length = len(value)
            elif i != 0 and length != len(value):  # OR elif length != len(value):
                raise ValueError("All arrays must be the same length")

    def _convert_unicode_to_object(self, data):  # Unicode data type is not flexible.
        new_data = {}
        for key, value in data.items():
    ...
    def __len__(self):
        ...
        """
        # return len(next(iter(self._data.values()))) OR
...
class StringMethods:
    ...
    def _str_method(self, method, col, *args):
        ...
        if value.dtype.kind != 'O':
            raise TypeError('The `str` accessor only works with strings')
